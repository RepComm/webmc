...
export class FlatTexMesh extends WorldComponent {
  ...
  rgbaIsTransparent (v: rgba): boolean {
    return v.a < this.alphaCutoff;
  }
  rgbaPick (out: rgba, x: number, y: number) {
  ...
  neighbors (sides: MeshBuilderCubeSides, x: number, y: number) {
  ...
  setImage (url: string): Promise<void> {
    return new Promise(async (_resolve, _reject)=>{
      ...
      img.onload = ()=>{
        ...
        this.imgdata = FlatTexMesh.ctx.getImageData(0, 0, img.width, img.height);
        
        this.texture = new Texture(Globals.gl, {
          image: img,
          magFilter: Globals.gl.NEAREST,
          minFilter: Globals.gl.NEAREST,
          // format: Globals.gl.RGBA,
          // premultiplyAlpha: true,
          
        });

        this.shader = new Program(Globals.gl, {
          transparent: true,
          vertex: `
            attribute vec2 uv;
